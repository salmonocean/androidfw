...
bool ResTable_config::isMoreSpecificThan(const ResTable_config& o) const {
    ...
    if (uiMode || o.uiMode) {
        ...
        //[+LEUI][dongshangyong]: eui oled mode
        if (((uiMode^o.uiMode) & MASK_UI_MODE_EUI_OLED) != 0) {
            if (!(uiMode & MASK_UI_MODE_EUI_OLED)) return false;
            if (!(o.uiMode & MASK_UI_MODE_EUI_OLED)) return true;
        }
        //[-LEUI]
    }
...
bool ResTable_config::isBetterThan(const ResTable_config& o,
    ...
    if (requested) {
        ...
        if (uiMode || o.uiMode) {
            ...
            //[+LEUI][dongshangyong]: eui oled mode
            if (((uiMode^o.uiMode) & MASK_UI_MODE_EUI_OLED) != 0
                    && (requested->uiMode & MASK_UI_MODE_EUI_OLED)) {
                return (uiMode & MASK_UI_MODE_EUI_OLED);
            }
            //[-LEUI]
        }
...
bool ResTable_config::match(const ResTable_config& settings) const {
    ...
    if (screenConfig != 0) {
        ...
        //[+LEUI][dongshangyong]: eui oled mode
        const int uiModeEuiOLED = uiMode&MASK_UI_MODE_EUI_OLED;
        const int setUiModeEuiOLED = settings.uiMode&MASK_UI_MODE_EUI_OLED;
        if (uiModeEuiOLED != 0 && uiModeEuiOLED != setUiModeEuiOLED) {
            return false;
        }
        //[-LEUI]
...
String8 ResTable_config::toString() const {
    ...
    //[+LEUI][dongshangyong]: eui oled mode
    if ((uiMode & MASK_UI_MODE_EUI_OLED) != 0) {
        if (res.size() > 0)
            res.append("-");
        switch (uiMode & ResTable_config::MASK_UI_MODE_EUI_OLED) {
            case ResTable_config::UI_MODE_EUI_OLED_NO:
                res.append("notoled");
                break;
            case ResTable_config::UI_MODE_EUI_OLED_YES:
                res.append("oled");
                break;
            default:
                res.appendFormat("uiModeEuiOLED=%d",
                    dtohs(uiMode&MASK_UI_MODE_EUI_OLED));
                break;
        }
    }
    //[-LEUI]
